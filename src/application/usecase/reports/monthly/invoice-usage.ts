...
export default class InvoiceUsageMonthly implements CommandHandler<Promise<output[]>, null> {

    constructor(private readonly repository: InvoiceRepository) { }

    async handler(): Promise<output[]> {
        const months = this.getLastMonths();
        const report = await this.repository.getLastYear();
    ...
    private getMonthTotal(report_items: any[]): { quantity: number, total_price: number } {
        ...
        return report_items.reduce((acc, item) => {
